...
export const getCollections = async (params: { locale: string; channel: string }) => {
      ...
      _collections.collections.items.map(async (c) => {
        const products = await SSGQuery(params)({
          collection: [
            { slug: c.slug },
            {
              productVariants: [{ options: { take: 4, sort: { createdAt: SortOrder.ASC } } }, { totalItems: true, items: CollectionTileProductVariantSelector }],
            },
          ],
        });
  ...
  const collections = _collections.collections.items.map((c) => {
    const collection = variantForCollections.length ? variantForCollections.find((p) => p.id === c.id) : { productVariants: { items: [], totalItems: 0 } };
