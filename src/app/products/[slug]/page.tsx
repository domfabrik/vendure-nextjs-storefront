...
import { buildBreadcrumbJsonLd, buildProductJsonLd, generateProductMetadata, getProductCollectionChain } from '@/entities/product/index.server';
...
export default async function Page(props: PageProps) {
  ...
  const canonicalCollection = collectionChain[collectionChain.length - 1];
  const relatedCollectionSlug = canonicalCollection?.slug;
  ...
  return (
    <Box>
      ...
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        {collectionChain.map((collection, index) => (
          <NextLink
            key={collection.slug}
      ...
      <ProductDetailEvent
        ...
        category={canonicalCollection?.name}
