...
>((initialState) => {
  ...
  const [filters, setFilters] = useState<{ [key: string]: string[] }>(initialState.filters ? initialState.filters : {});
  ...
  useEffect(() => {
    ...
    if (query && Object.keys(query).filter((k) => k !== 'slug' && k !== 'locale' && k !== 'channel').length) {
      ...
      setFilters(filters);
      getFilteredProducts(filters, query.page ? parseInt(query.page as string, 10) : 1, sort, q);
    }
  }, [query]);
