  ...
  const resp = await Promise.all(
    allPaths.map(async (path) => {
      ...
      const data = await SSGQuery({ channel, locale: path.params.locale })({
        collections: [{ options: { filter: { slug: { notEq: 'search' } }, topLevelOnly: true } }, { items: { id: true, slug: true, children: { id: true, slug: true } } }],
      }).then((d) => {
        const routes = d.collections?.items.flatMap((c) => {
