...
export const Link = ({ children, skipLocaleHandling, external, ariaLabel, ...rest }: PropsWithChildren<LinkComponentProps>) => {
  ...
  // }
  const _channel = channel ? (channel === DEFAULT_CHANNEL_SLUG && !router.query.locale ? `` : `/${router.query.channel}`) : '';
  if (_channel && !skipLocaleHandling) {
    linkHref = href ? `${_channel}${locale ? `/${locale}` : ''}${linkHref}` : router.pathname.replace('/[channel]', _channel).replace('/[locale]', locale ? `/${locale}` : '');
  }
