  ...
  const onSubmit: SubmitHandler<FormValues> = (data) => {
    ...
    if (sameAsChannel) {
      if (haveChannel && haveLocale) {
        ...
        const correctPathname = (split[0] + (newLang === channelAsLocale.slug ? '' : newLang) + split[1]).replace(
          '[channel]',
      ...
      if (haveChannel && !haveLocale) {
        ...
        const correctPathname = split[0] + (channelAsLocale.slug === DEFAULT_CHANNEL_SLUG ? '' : channelAsLocale.nationalLocale) + split[1];

        console.log(correctPathname);
        push(correctPathname);
      }
      if (!haveChannel && !haveLocale) {
        const _channel = channelAsLocale?.channel === DEFAULT_CHANNEL ? '' : channelAsLocale?.nationalLocale === channelAsLocale.slug ? '' : channelAsLocale?.nationalLocale;
      ...
      if (haveChannel && haveLocale) {
        const split = preparedPathname.split('[locale]');
        const correctPathname = (split[0] + newLang + split[1]).replace('[channel]', channelAsLocale?.slug ?? '');
      ...
      if (!haveChannel && !haveLocale) {
        ...
        const _channel = channelAsLocale?.channel === DEFAULT_CHANNEL && newLang === DEFAULT_CHANNEL_SLUG ? '' : channelAsLocale?.nationalLocale;
                ...
                render={({ field: { value, onChange } }) => (
                  <Dropdown
                    ...
                    setSelected={(channel) => {
                      ...
                      setValue('locale', channels.find((c) => c.nationalLocale === channel)?.nationalLocale as string);
                    }}
                    selected={value}
