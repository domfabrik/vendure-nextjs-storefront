...
export const ProductOptions = ({ productOptionsGroups, handleClick, addingError }: ProductOptionsProps) => {
  return (
    <Stack
      column
      gap="2.5rem"
...
const SizeSelector = styled(VariantButton)<{ selected: boolean; outOfStock: boolean }>`
    ...
    ${(p) => (p.selected ? `background: ${p.theme.button.back}; color: ${p.theme.gray(0)};` : p.outOfStock && `opacity: 0.5;`)}
`;
