...
export const useSlider = ({ loop = true, spacing }: { loop?: boolean; spacing: number }) => {
  ...
  const goToSlide = (slide: number) => (jsEnabled ? slider.current?.moveToIdx(slide) : console.log('JavaScript is not enabled'));

  return { jsEnabled, ref, slider, nextSlide, prevSlide, goToSlide, currentSlide };
};

function refreshSlider(sliderInstanceRef: React.MutableRefObject<KeenSliderInstance | null>, sliderOptionsValue: KeenSliderOptions) {
  return () => {
