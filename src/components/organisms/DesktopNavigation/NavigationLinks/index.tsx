...
export const NavigationLinks = ({ collection }: { collection: RootNode<CollectionTileType>['children'][number] }) => {
  return (
    <Stack
      column
      gap="1.5rem"
    >
      ...
      <Stack column>
        {collection.children.map((cc) => {
          const href = cc.parent?.slug === '__root_collection__' ? `/collections/${cc.slug}` : `/collections/${cc.parent?.slug}/${cc.slug}`;
          return (
            <Stack
              key={`${cc.name}1`}
