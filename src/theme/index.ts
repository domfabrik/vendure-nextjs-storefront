...
const themeTransform = (t: MainTheme) => {
  ...
  const { gray: _gray, accent: _accent, borderRadius: _borderRadius, grayAlpha: _grayAlpha, ...rest } = t;
  const deepRestCopy = JSON.parse(JSON.stringify(rest));
  tree(deepRestCopy);
  return deepRestCopy as Gen<DetailTheme>;
};

export const createTheme = (hue: number, fn: (theme: FunctionTheme) => DetailTheme, themeFunction = defaultThemeFunction): MainTheme => {
